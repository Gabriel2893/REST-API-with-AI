# 💻 Criação de um projeto de API REST com IA
### 🌐 English text bellow 
## 📚 Sobre o Projeto

Neste repositório, encontra-se o código fonte de uma API REST, construída com Java 21 e Spring Boot 3, (desenvolvida durante a Santander Dev Week 2024)

> [!NOTE]
> Objetivo: "Permitir que usuários conversem com campeões de League of Legends (LOL)".

Para isso, utilizei algumas das mais recentes Inteligências Artificiais Gerativas (IAs), permitindo que nossa API "entenda" a personalidade única de cada campeão para criar interações que capturem sua essência, tornando cada conversa uma experiência única.
...
Refletindo a organização apresentada no diagrama arquitetural, a estrutura de directórios do projeto sugere uma Arquitetura Limpa simplificada, visando uma clara separação de responsabilidades e promovendo a autonomia das camadas num projeto Spring Boot. Essa abordagem estrutural não apenas facilita a manutenção e a evolução do código, mas também suporta a integração e a colaboração efetiva entre as diferentes partes da aplicação. Abaixo, detalhamos o layout dos diretórios que compõem a aplicação, cada um desempenhando um papel específico dentro do ecossistema de software:

- `adapters/`: Inclui os adaptadores que facilitam a comunicação entre a aplicação e o mundo exterior (o único diretório que "conhece" o Spring).
    - `in/`: Abriga os adaptadores de entrada, como os controladores REST, que lidam com as solicitações do usuário.
    - `out/`: Contém os adaptadores de saída, responsáveis por interagir com bancos de dados e APIs externas, por exemplo.
- `application/`: Hospeda os casos de uso do aplicativo, encapsulando a lógica comercial essencial.
- `domain/`: Representa o coração da aplicação, compreendendo entidades, exceções e interfaces (ports) que articulam as regras fundamentais do negócio.
    - `exception/`: Define as excepções personalizadas relevantes para o domínio.
    - `model/`: Modela as entidades do domínio, reflectindo os conceitos centrais da aplicação.
    - `ports/`: Estabelece as interfaces que definem os contratos para adaptadores e serviços externos.
- `Application.java`: A classe principal que orquestra a configuração e execução da aplicação.

### Base de dados SQL em memória

A utilização da base de dados H2 neste projeto serve como uma base ágil e flexível para modelar o nosso domínio de conhecimento - os campeões de LOL. Esta escolha permite uma prototipagem rápida e um ambiente de desenvolvimento eficiente, o que é essencial para armazenar e recuperar informações detalhadas sobre cada campeão. Desta forma, garantimos que as IAs generativas que integramos podem aceder a um repositório rico e detalhado, permitindo-lhes captar com precisão a essência e a personalidade única de cada campeão, enriquecendo assim a interatividade e a profundidade das interacções realizadas.
